    	 ...
    	 if (!file.isEmpty()) {
				try {
					...
					m.addAttribute( "categoryList", categoryList);
				      ...
				      /*  Set<Products> productAs = new Set<Products>(){{
				            add(new Products("Book A1", categoryA));
				            add(new Products("Book A2", categoryA));
				            add(new Book("Book A3", categoryA));
				        }};
				        categoryA.setBooks(productAs);*/
					productService.addProducts(products,products.getName());
					 CategoryEntity categoryA = new  CategoryEntity();
					  categoryA.setProducts(productService.getProductsByCategory(products.getCategory()));
					  adminService.addSupplier(categoryA);
...
public String productDis(Model model)
{
